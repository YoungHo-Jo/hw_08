...
DMA_InitTypeDef DMA_InitStructure;
 ...
 {
     // initialize LED C8, 9
	 myGPIO.GPIO_Pin = GPIO_Pin_1 | GPIO_Pin_2| GPIO_Pin_3 | GPIO_Pin_4 | GPIO_Pin_5; //set to pin 1, 2
	 myGPIO.GPIO_Mode = GPIO_Mode_AIN; //set as analog input
	 GPIO_Init(GPIOC, &myGPIO); //set to C1, C2
 }

void adc_config(uint32_t * adc_data)
{
  ...
  ADC_RegularChannelConfig(ADC1, ADC_Channel_13, 3, ADC_SampleTime_55Cycles5); //PC2 as Input
  ADC_RegularChannelConfig(ADC1, ADC_Channel_14, 4, ADC_SampleTime_55Cycles5); //PC2 as Input
  ADC_RegularChannelConfig(ADC1, ADC_Channel_15, 5, ADC_SampleTime_55Cycles5); //PC2 as Input
